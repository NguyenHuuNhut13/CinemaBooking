...
namespace CinemaBooking
{
    public class QRPay
    {
        ...
        public string Build()
        {
            ...
            string EVMCoContent = string.Join("", (this.EVMCo ?? new Dictionary<string, string>()).Keys.OrderBy(key => key).Select(key => QRPay.GenFieldData(key, this.EVMCo?.GetValueOrDefault(key))));
            string unreservedContent = string.Join("", (this.unreserved ?? new Dictionary<string, string>()).Keys.OrderBy(key => key).Select(key => QRPay.GenFieldData(key, this.unreserved?.GetValueOrDefault(key))));
